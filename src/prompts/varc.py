...
There are exactly {len(rc_groups)} RC passage groups in this PDF: {groups_str}. Each is preceded by an "Instructions [N - M]" header.

The response schema has two top-level fields:

**passage_groups** — exactly {len(rc_groups)} entries, one per RC group above, in order. For each:
- start, end: the integers N and M for that group.
...
**other_questions** — every question not in any of the {len(rc_groups)} RC groups above. Sub-types:
